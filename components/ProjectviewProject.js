// components/ProjectViewProject.js
"use client";
...
function computeCompletion(project) {
  ...
  total += 1;
  if (project.nextTrigger && project.nextTrigger.trim() !== "") filled += 1;

  total += 1;
  if (project.prevTrigger && project.prevTrigger.trim() !== "") filled += 1;

  total += 1;
  if (project.stopTrigger && project.stopTrigger.trim() !== "") filled += 1;

  total += 1;
  if (Array.isArray(project.queue) && project.queue.length > 0) filled += 1;

  total += 1;
  if (project.pptxUrl && project.pptxUrl.trim() !== "") filled += 1;
...
export default function ProjectViewProject({
  maxHeight,
  currentProjectId,
  onOpenProject,
  refreshTrigger,
  ...
  useEffect(() => {
    async function loadProjects() {
      try {
        ...
        if (!res.ok) {
          console.error("Failed to fetch projects in ProjectViewProject");
          return;
        }
        ...
        console.error("Error loading projects in ProjectViewProject:", err);
      }
    }
  ...
  // Keep local selection in sync with the currently opened project
  useEffect(() => {
  ...
  return (
    <div
      className="w-full h-full rounded-2xl border border-white/10 shadow-[0_18px_45px_rgba(10,20,70,0.75)] flex flex-col min-h-0"
      style={{
    ...
    >
      <div className="flex items-center justify-between mb-4 flex-none">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-white font-semibold text-[20px]">All Projects</h3>
        </div>
      </div>

      {projects.length === 0 ? (
        <div className="flex-1 min-h-0 text-sm text-white/60">
          No projects yet.
        </div>
      ) : (
            ...
            return (
              <motion.button
                ...
                onClick={() => {
                  setSelectedId(id);
                  if (onOpenProject) {
                    onOpenProject(id); // 🔥 tell DashboardLayout to open it
                  }
                }}
              ...
              >
                ...
                <div className="text-xs text-white/70">{pct}% Complete</div>
              </motion.button>
            );
          })}
          </AnimatePresence>
